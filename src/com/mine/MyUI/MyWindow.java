...
import com.mine.arrayListTools.MyArrayList;

public class MyWindow extends Window implements MyUI {
	/**
	 * 
	 */
	...
	MyWindow(Window win) throws HeadlessException {
		super(win);
		init();
	}

	MyWindow(Window win, GraphicsConfiguration gc) {
		super(win, gc);
		init();
	}

	public MyWindow(WindowTheme theme, Window win) {
		super(win);
		this.theme = theme;
		theme.init(this);
		init();
	...
	private class FrameListener implements MouseListener, MouseMotionListener {
		...
		private Window mframe;
		private boolean moveInformation = false;

		public FrameListener(Window mframe) {
			this.mframe = mframe;
		}
